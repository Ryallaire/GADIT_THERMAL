...
//== Added by Ryan H. Allaire to copy data from w_tranpose solution to v
__global__ void transfer_solution(
	double *vec_in,				// Array containing solution
	double *vec_out,			// Array that you want solution
	int n,						// Size of linear systems
	int batchCount				// Number of linear systems
)
{
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	int y = blockDim.y * blockIdx.y + threadIdx.y;
	int index_non_padded;
	int index_transposed;

	if(x < n && y < batchCount){
		index_non_padded = y * n + x;
		index_transposed = x * batchCount + y;
		// index_transposed = index_non_padded;
		vec_out[index_non_padded] = vec_in[index_transposed];
	}
}

__global__ void transpose_vectors_ryan(
	double *vec_in,				// Array containing solution
	double *vec_out,			// Array that you want solution
	int n,						// Size of linear systems
	int batchCount				// Number of linear systems
