		...
		DATATYPE* d_h, int  const  k0, int  const  l0, int const k0_l0 )
	{
			...
			case boundary_condtion_type::SYMMETRIC:
				if (k0 == 0)
				{
					...
					d_ws.f3[km_l0] = d_ws.f3[k0_l0];			//Added boundary conditions for f3=Dh^2
					d_ws.df3[km_l0] = d_ws.df3[k0_l0];			//=====================================
					
					d_ws.gamma[km_l0] = d_ws.gamma[k0_l0];		//Added symmetry condition on surface tension based on Tx,Ty=0 at boundaries
					d_ws.visc_rat[km_l0] = d_ws.visc_rat[k0_l0];
				}
		...
		DATATYPE* d_h, int  const  k0, int  const  l0, int const k0_l0)
	{
			...
			case boundary_condtion_type::SYMMETRIC:
				...
				if (k0 == (dims.n-1))
				{
					...
					d_ws.f3[kp_l0] = d_ws.f3[k0_l0];
					d_ws.df3[kp_l0] = d_ws.df3[k0_l0];
					
					d_ws.gamma[kp_l0]=d_ws.gamma[k0_l0];		//Added this for surface tension gamma_x=0
					d_ws.visc_rat[kp_l0]=d_ws.visc_rat[k0_l0];
		...
		DATATYPE* d_h, int  const  k, int  const  l0, int const k0_l0)
	{
			...
			case boundary_condtion_type::SYMMETRIC:
				if (l0 == 0)
				{
					...
					d_ws.f3[k0_lm] = d_ws.f3[k0_l0];
					d_ws.df3[k0_lm] = d_ws.df3[k0_l0];
					
					d_ws.gamma[k0_lm] = d_ws.gamma[k0_l0];			//Added this condition for gamma_y =0 at y=0
					d_ws.visc_rat[k0_lm] = d_ws.visc_rat[k0_l0];	
		...
		DATATYPE* d_h, int  const  k0, int  const  l0, int const k0_l0)
	{
			...
			case boundary_condtion_type::SYMMETRIC:
					if (l0 == dims.m - 1)
					{
						...
						d_ws.f3[k0_lp] = d_ws.f3[k0_l0];
						d_ws.df3[k0_lp] = d_ws.df3[k0_l0];

						d_ws.gamma[k0_lp]=d_ws.gamma[k0_l0];		//Added this for gamma_y=0 at y=YM
						d_ws.visc_rat[k0_lp]=d_ws.visc_rat[k0_l0];
...
#pragma endregion rhs_vector_bc
	...
	template <typename DATATYPE, bc_postition BC_POS ,boundary_condtion_type BC_START, boundary_condtion_type BC_END> __device__ __forceinline__
		void lhs_matrix_J
			...
			DATATYPE  &HSS_L0_L0,		DATATYPE  &HSS_LM_L0,
			DATATYPE  &HSS_LP_L0,		DATATYPE  &HSS_L0_LP,
			DATATYPE  &HSS_LM_LP,		DATATYPE  &HSS_LP_LP,
			DATATYPE  &HSS_LMM_LP,
		...
		if (BC_POS == bc_postition::INTERIOR)
		{
			...
			HSS_LM_L0 = 1.0;		HSS_LM_LP = 1.0;
			HSS_L0_L0 = -2.0;		HSS_L0_LP = -2.0;
			HSS_LP_L0 = 1.0;		HSS_LP_LP = 1.0;
									HSS_LMM_LP = 0;
				...
				switch (BC_START)
				{
					case boundary_condtion_type::SYMMETRIC:
						...
						case bc_postition::FIRST:
							...
							HSSS_LM_L0 = 0.0;			HSSS_LM_LP = 2.0;
							HSSS_LMM_L0 = 0.0;		HSSS_LMM_LP = 0.0;

							HSS_LM_L0 = 0.0;			HSS_LM_LP = 1.0;
							HSS_L0_L0 = -1.0;		HSS_L0_LP = -2.0;
							HSS_LP_L0 = 1.0;		HSS_LP_LP = 1.0;
													HSS_LMM_LP = 0.0; //Not needed here- just defining for use later
						...
						case bc_postition::THIRD:
							...
							HSS_LM_L0 = 1.0;		HSS_LM_LP = 1.0;  //NOTE THAT IF NO BC IS APPLIED THEN STENCILS L0 AND LP HAVE SAME DERIVATIVES
							HSS_L0_L0 = -2.0;		HSS_L0_LP = -2.0;
							HSS_LP_L0 = 1.0;		HSS_LP_LP = 1.0;
													HSS_LMM_LP = 0.0; //Not used yet
			...
			else if ((BC_POS == bc_postition::FIRST_LAST) || (BC_POS == bc_postition::SECOND_LAST) || (BC_POS == bc_postition::THIRD_LAST))
			{
				...
				case boundary_condtion_type::SYMMETRIC:
					...
					case bc_postition::FIRST_LAST:
						HSSS_LP_L0 = 0.0;		HSSS_LP_LP = 0.0;
						HSSS_L0_L0 = -2.0;		HSSS_L0_LP = 0.0;
						HSSS_LM_L0 = 3.0;		HSSS_LM_LP = 0.0;
						HSSS_LMM_L0 = -1.0;		HSSS_LMM_LP = 0.0;
						
						HSS_LM_L0 = 1.0;		HSS_LM_LP = -1.0;
						HSS_L0_L0 = -1.0;		HSS_L0_LP = 0.0;
						HSS_LP_L0 = 0;			HSS_LP_LP = 0.0; 	
												HSS_LMM_LP = 1.0;	//Note that this value was changed for the right (upper) end points
					...
					case bc_postition::SECOND_LAST:
						...
						HSS_LM_L0 = 1.0;		HSS_LM_LP = 1.0;
						HSS_L0_L0 = -2.0;		HSS_L0_LP = -1.0;
						HSS_LP_L0 = 1.0;		HSS_LP_LP = 0.0; 	
												HSS_LMM_LP = 0;
