...
namespace file_directories
{
		...
		// std::string summitdir			= "var_surf_var_visc_var_TC/";
		std::string summitdir			= "";									// Top directory for cluster file system
		std::string tmpDir				= summitdir + "temp";
		std::string statusDir			= summitdir + "status";
		std::string inputDir			= summitdir + "input";
		std::string outputDir			= summitdir + "output";
		...
		std::string backupfilmtemp		= tmpDir	+ "/BackupFilmTemp.bin";
		std::string backupsubtemp		= tmpDir	+ "/BackupSubTemp.bin";
		...
		void make_directories( std::string root )
		{
			...
			std::experimental::filesystem::create_directories(temp_file_dir);
		...
		void clean()
		{
			...
			remove(backupfilmtemp.c_str());
			remove(backupsubtemp.c_str());
		}
...
template <typename DATATYPE> void output_binary2(const std::string& filename, DATATYPE **data, size_t n, size_t m)
{
	ofstream output(filename.c_str(), ios::out | ios::binary | ios::app);
	for (int j = 0; j < m; j++) 
	{
		output.write((char *)data[j], n* sizeof(DATATYPE));
	}
