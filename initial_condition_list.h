...
namespace ic_linear_waves
{
		...
		// Define the general template structure
...
/*	template<typename DATATYPE, initial_condition::id IC_ID> DATATYPE
		compute(DATATYPE &x, DATATYPE &y,
				spatial_parameters<DATATYPE> parasSpatial, initial_parameters<DATATYPE> parasInitial){ return 0;} */

	template<typename DATATYPE, initial_condition::id IC_ID = ID> DATATYPE
		compute(DATATYPE &x, DATATYPE &y,
				spatial_parameters<DATATYPE> parasSpatial, initial_parameters<DATATYPE> parasInitial)
	{
		return parasInitial.h0*( 1.0 + parasInitial.epx*( cos (parasInitial.nx*PI*x/(parasSpatial.xn-parasSpatial.x0) ) )
...
namespace initial_condition_list{
	...
	template<typename DATATYPE, initial_condition::id IC_ID> load_status compute(DATATYPE *x, DATATYPE *y, DATATYPE *h, dimensions dims ,
		spatial_parameters<DATATYPE> parasSpatial, initial_parameters<DATATYPE> parasInitial , io_parameters parasIo)
	{
		...
		{
			bool is_file_loaded;
			bool is_file1_loaded;
			//bool is_file2_loaded;
			is_file1_loaded = load_binary (parasIo.root_directory + file_directories::icInputFile , h , dims.n_pad , dims.m_pad );
			//is_file2_loaded = load_binary (parasIo.root_directory + file_directories::icInputFile , res_temp , dims.n_pad , dims.m_pad );
			is_file_loaded = is_file1_loaded;// && is_file2_loaded;
