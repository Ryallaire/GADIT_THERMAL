...
#pragma endregion up_solve
		...
		lu_decomposition(reduced_device_workspace<DATATYPE> d_ws, dimensions dims )//, newton_status volatile &status)
	{
			...
			// printf("Jy_a=%12.9e\n", pent_ws.LU_a[globalIdx_transpose]);

			int globalIdx_stored = globalIdx;
			int globalIdx_transpose_stored = globalIdx_transpose;
			...
			// if(globalIdx == 0 && Y_DIRECTION){
			// 	for(int k=0; k<5791; k++){
			// 		printf("Jy_c[k]=%23.17e\t k = %d\n", d_ws.Jy_c[k], k);
			// 	}
			// }

			// //== Temporarily store Jy values
			// if(Y_DIRECTION){
			// 	if(globalIdx_transpose_stored==m-1-5){
			// 		r_Jy_a = d_ws.Jy_a[globalIdx_transpose]; 
			// 		r_Jy_b = d_ws.Jy_b[globalIdx_transpose]; 
			// 		r_Jy_c = d_ws.Jy_c[globalIdx_transpose]; 
			// 		r_Jy_d = d_ws.Jy_d[globalIdx_transpose]; 
			// 		r_Jy_e = d_ws.Jy_e[globalIdx_transpose]; 
			// 	}
			// }
			...
			// __syncthreads();
			// if(globalIdx_stored == 90 && Y_DIRECTION){
			// 	// printf("w[2805]=%23.17e\t index = %d\n", pent_ws.y_transpose[(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x],(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x);	
			// 		// printf("w[2895]=%23.17e\t index = %d\n", d_ws.w_transpose[2895],(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x);
			// 		for(int k=0; k<32; k++){
			// 			// printf("v[k]=%23.17e\t index = %d\n", pent_ws.y_transpose[k],k);
			// 			printf("w[k]=%23.17e\t index = %d\n", pent_ws.y_transpose[k],k);						
			// 		}
			// 		// printf("w[0]=%23.17e\t index = %d\n", d_ws.w_transpose[10],(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x);
			// // 
			// }			
			// if(globalIdx_stored == 0 && Y_DIRECTION == false){
			// 	// printf("w[2805]=%23.17e\t index = %d\n", pent_ws.y_transpose[(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x],(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x);	
			// 		// printf("w[2895]=%23.17e\t index = %d\n", d_ws.w_transpose[2895],(blockIdx.x*THREAD_SIZE)*m + m - 1 - threadIdx.x);
			// 		for(int k=0; k<181; k++){
			// 			printf("v[k]=%23.17e\t index = %d\n", pent_ws.y_transpose[k],k);						
			...
			// 	// if(threadIdx.x == 5){
			// 		if(globalIdx_transpose_stored==m-1-5){
			// 		penta_res = r_Jy_a * pent_ws.y_transpose[globalIdx_transpose_stored-2];
			// 		penta_res += r_Jy_b * pent_ws.y_transpose[globalIdx_transpose_stored-1];
			// 		penta_res += (r_Jy_c-1) * pent_ws.y_transpose[globalIdx_transpose_stored];
			// 		penta_res += r_Jy_d * pent_ws.y_transpose[globalIdx_transpose_stored+1];
			// 		penta_res += r_Jy_e * pent_ws.y_transpose[globalIdx_transpose_stored+2];
			// 		// penta_res = d_ws.Jy_a[globalIdx_transpose_stored] * pent_ws.y_transpose[globalIdx_transpose-2+THREAD_SIZE];
			// 		// penta_res += d_ws.Jy_b[globalIdx_transpose_stored] * pent_ws.y_transpose[globalIdx_transpose_stored-1];
			// 		// penta_res += (d_ws.Jy_c[globalIdx_transpose_stored]-1) * pent_ws.y_transpose[globalIdx_transpose_stored];
			// 		// penta_res += d_ws.Jy_d[globalIdx_transpose_stored] * pent_ws.y_transpose[globalIdx_transpose_stored+1];
			// 		// penta_res += d_ws.Jy_e[globalIdx_transpose_stored] * pent_ws.y_transpose[globalIdx_transpose_stored+2];

			// 		printf("penta_res = %12.9e\t %d\n", penta_res, globalIdx_transpose);
			// 		// printf("thread_size= %d\n",THREAD_SIZE);
			// 	}
			// }
			// } 
			//== Uses transpose values for Jx_a,b,c,d,e
			// if(Y_DIRECTION==true && globalIdx_transpose_stored==m-1-5){
			// 		printf("w[2805]=%12.9e\n", pent_ws.y_transpose[globalIdx_transpose_stored]);
			// }
		}
