...
public:
	...
	void update_dt_max(double dt_max)
	{
		parameters.dt_max = dt_max;
	}
	...
	timestep_manager_status::status update_dt(  newton_status::status &status )
	{
		...
		else if ( status == newton_status::FROZEN )
		{
			time_steps++;
			// stable_time_steps++;
		...
		{
			//== This routine gets called when Temp_Change_dt is true or newton status is not a success.
			stable_time_steps = 0;
			is_at_max_time_step = false;
